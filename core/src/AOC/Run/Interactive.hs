...
execSolution cs = eitherIO $ do
    ...
    out <- mainRun cfg . defaultMRO $ TSPart cs
...
viewPrompt cs = eitherIO $ do
    cfg <- liftIO $ configFile defConfPath
    out <- mainView cfg . defaultMVO $ TSPart cs
...
submitSolution :: ChallengeSpec -> SomeSolution -> IO (Text, SubmitRes)
submitSolution cs ss = eitherIO $ do
    cfg <- liftIO $ configFile defConfPath
    mainSubmit cfg ss cs defaultMSO
...
submitSolution_ :: SomeSolution -> IO ()
...
mkSpec :: Integer -> Integer -> Part -> ChallengeSpec
mkSpec y i = CS y (mkDay_ i)
